...
import type { ClientMessage, SendClientMessage, ServerMessage } from '@shared/shared-message-and-session-types'
import type { ConnectionStatus } from '../stores/session-state-store'
...
export class WebSocketManager {
  ...
  connect() {
    ...
    const backendHost = import.meta.env.DEV
      ? `${window.location.hostname}:${import.meta.env.VITE_WS_PORT || '4040'}`
      : window.location.host
    const wsUrl = `${scheme}://${backendHost}/ws`
