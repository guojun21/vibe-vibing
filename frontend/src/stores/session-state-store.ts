...
import type { AgentSession, HostStatus, Session } from '@shared/shared-message-and-session-types'
import { sortSessions } from '../utils/session-sorting-utilities'
import { useSettingsStore } from './settings-persistence-store'
import { createTabStorage } from '../utils/tab-scoped-local-storage'
...
export const useSessionStore = create<SessionState>()(
  persist(
    (set, get) => ({
      ...
      unpinSession: (id) => set((state) => ({
        pinnedSessionIds: state.pinnedSessionIds.filter((sid) => sid !== id),
      })),
