...
import type { AgentSession, Session } from '@shared/shared-message-and-session-types'
import { getSessionOrderKey, getUniqueHosts, getUniqueProjects, sortSessions } from '../utils/session-sorting-utilities'
...
import { getSessionIdShort } from '../utils/session-identifier-utilities'
import { composeSortableTransform } from '../utils/drag-sortable-transform-modifier'
import { useSettingsStore } from '../stores/settings-persistence-store'
import { useSessionStore } from '../stores/session-state-store'
import { getEffectiveModifier, getModifierDisplay } from '../utils/device-detection-and-modifier-keys'
import { useCounterBump } from '../hooks/use-counter-bump-animation'
import { useExitCleanup } from '../hooks/use-exit-animation-cleanup'
import AgentIcon from './agent-type-icon-badge'
import InactiveSessionItem from './inactive-session-list-item'
import ProjectBadge from './project-path-color-badge'
import HostBadge from './remote-host-status-badge'
import HostFilterDropdown from './host-filter-dropdown-menu'
import ProjectFilterDropdown from './project-filter-dropdown-menu'
import SessionPreviewModal from './session-preview-modal-dialog'
...
interface SessionListProps {
  ...
  /** When true, renders without <aside> wrapper, header, and shortcuts — for embedding inside another scrollable container */
  embedded?: boolean
}
...
export default function SessionList({
  ...
  const renderSessionRows = () => (
    <>
      {loading ? (
        ...
        !embedded ? (
          <div className="px-3 py-6 text-center text-xs text-muted">No sessions</div>
        ) : null
      ) : (
        <DndContext
          sensors={sensors}
  ...
  if (embedded) {
    return <div>{renderSessionRows()}</div>
  }
