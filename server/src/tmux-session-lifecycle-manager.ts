...
import { inferAgentType } from './agent-type-detection-from-process'
import { config } from './server-configuration-loader'
import { normalizeProjectPath } from './agent-log-file-discovery'
...
import {
  ...
  inferSessionStatus,
  type PaneCacheState,
} from './terminal-status-inference-engine'
import type { Session, SessionStatus } from './shared/shared-message-and-session-types'
...
export class SessionManager {
  ...
  ensureSession(): void {
    ...
    try { this.runTmux(['set-option', '-t', this.sessionName, 'prefix2', 'None']) } catch {}
...
export { detectsPermissionPrompt } from './terminal-status-inference-engine'
